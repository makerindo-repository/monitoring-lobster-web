...
<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-4">
            <div class="card">
                <div class="card-body">
                    <h5 class="fw-bold mb-0">Edit data Kabupaten/Kota</h5>
                    ...
                    <form action="{{ route($route . 'update', $data->id) }}" method="POST">
                        ...
                        <div class="form-group mb-2">
                            ...
                            <input type="text" name="name" class="form-control" value="{{ $data->name }}" placeholder="Masukan Nama kabupaten atau kota" required>
                        </div>
                        <div class="form-group mb-2">
                            <label for="">Provinsi</label>
                            <select name="region_id" placeholder="Pilih Provinsi" autocomplete="off" required>
                                @foreach($regions as $id => $name)
                                <option value="{{ $id }}" {{ $data->region_id == $id ? 'selected' : '' }}>{{ $name }}</option>
                                @endforeach
                            </select>
                        </div>
